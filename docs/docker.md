...
The container exposes Traefik on TCP port `8080` and WebRTC ICE on UDP ports `50000-50010`. Put TLS or an external ingress in front of port `8080` when the public URL uses HTTPS, and allow or forward the UDP range to the Docker host for direct WebRTC connectivity.
