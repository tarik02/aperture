...
Live-session presentation state exposes remote cursor visibility and, when the compositor media producer is available, its WebRTC encoder profiles and current frame rate and bitrate. Cursor visibility, encoder profile, and quality are shared session state; owners and editors may change them, while viewers may only choose their client-local delivery transport. A profile change moves the requesting client to WebSocket before updating the producer, then negotiates a fresh WebRTC transport. Other incompatible WebRTC clients fall back and reconnect through the normal transport-recovery path.
