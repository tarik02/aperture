...
HTTP and MCP callers may retarget a running tab recording to another ready top-level target. Retargeting keeps the recording ID, output path, timeline, and encoder settings. Aperture starts the replacement Weston-output capture through PipeWire and GStreamer and waits for the new segment to produce data before stopping the old pipeline. Sending the current target is idempotent; viewer, stopped, and failed recordings reject explicit retargeting.

The formal HTTP surface provides create, list, get, retarget, and stop operations under `/api/sessions/{sessionId}/recordings`. Its `Recording` representation exposes only a session-relative output path; the wrapper's absolute filesystem path remains private. Stop continues to return the completed `SessionFile` rather than a recording status.
...
The HTTP API exposes `mode`, `targetId`, and `recordingId`. MCP status includes the same fields, while MCP start always selects tab mode. In both APIs, `targetId` identifies an Aperture top-level target. The existing singular start, status, and stop state will be replaced rather than overloaded with an implicit current target.
