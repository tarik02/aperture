...
`POST /sessions/:sessionId/uploads` takes `multipart/form-data`; every part with a filename is stored in the session's `uploads` directory. Names are sanitized and get a numeric suffix instead of overwriting. The `201` response is `{ "files": [...] }` with `name`, `path` (`uploads/<name>`, for `browser_file_upload`), `absolutePath` (for CDP `DOM.setFileInputFiles`), `sizeBytes`, and `modifiedAt`. Limits: 100 files per request, 1000 uploads per session, `session_upload_max_file_bytes` per file (`413`), and `session_storage_quota_bytes` per session (`507`). A rejected request stores nothing.
