...
export interface SessionViewportProps {
  ...
  className?: string;
  style?: CSSProperties;
  renderOverlay?: (overlay: SessionViewportOverlay) => ReactNode;
}

export type SessionViewportStatus = "webrtc" | "websocket" | "offline";

export type SessionViewportPlaceholder =
  | "connecting"
  | "connecting-media"
  | "switching"
  | "disconnected"
...
export interface SessionViewportOverlay {
  readonly status: SessionViewportStatus;
  readonly placeholder: SessionViewportPlaceholder | null;
  readonly cursorHint: { readonly text: string; readonly x: number; readonly y: number } | null;
  readonly followedCursor: {
...
export function SessionViewport({
  ...
  function handleKeyDown(event: React.KeyboardEvent) {
    ...
    if (clipboardShortcut === "paste") {
      void pasteClipboard(control, onNotice);
      return;
    }
  ...
  const placeholder = resolveViewportPlaceholder(
    control.phase,
    control.mediaPhase,
    control.mediaSwitching,
  );
  ...
  return (
    <div
      ...
      style={{
        ...
        outline: "none",
        touchAction: "none",
        cursor: localCursorEnabled ? undefined : "none",
        ...style,
      }}
    >
      {showingWebRTC ? (
        <div
          style={{
            ...
            opacity: mediaTransitioning ? 0.75 : 1,
      ...
      {renderOverlay?.({
        status,
        placeholder: showingWebRTC || frameMetadata ? null : placeholder,
        cursorHint:
          cursorHint && displayedCursorHintPoint
            ? { text: cursorHint, ...displayedCursorHintPoint }
            : null,
        followedCursor:
          followedCursorPoint && followedParticipant
            ? { name: followedParticipant.name, ...followedCursorPoint }
            : null,
        collaborationError: visibleCollaborationError?.message ?? null,
      })}
    </div>
  );
}
...
async function pasteClipboard(
  control: UseBrowserControlResult,
  onNotice: SessionViewportProps["onNotice"],
) {
  if (!control.activeTargetId) {
    return;
  }
  ...
  try {
    ...
    onNotice?.({ level: "error", message: "Clipboard read failed" });
    return;
  }
