...
Listing and download URLs need API credentials with `sessions:read`; uploads, deletes, and moves need `sessions:write`. Session-token holders use session-bound MCP or the data-plane upload. Editor and viewer capabilities cannot access files.
...
Successful uploads append a `session.file_uploaded` event containing the relative path, size, actor kind, and client IP. Deletes and moves through the API append `session.file_deleted` and `session.file_moved` events with the paths and client IP. The event never contains the session token or host storage path.
