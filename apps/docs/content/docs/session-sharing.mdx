...
Uploads accept the owner session token or an account token with `sessions:write`; the listing and download URLs need `sessions:read`. Editor and viewer capabilities cannot access files.
