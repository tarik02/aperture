...
The listing and download URLs are control-plane routes and work while the session is suspended or stopped. Signed downloads use attachment disposition and support HTTP byte ranges. Uploads reach the running session; they use `multipart/form-data` and may contain multiple files. Names are sanitized and collisions receive a numeric suffix instead of overwriting an existing file. The response lists the stored files with the same fields as the file listing.
...
Uploads accept the owner session token or an account token with `sessions:write`; the listing and download URLs need API credentials with `sessions:read`; session-token holders use session-bound MCP instead. Editor and viewer capabilities cannot access files.
