...
- `store_root`, `runtime_root` — persistent and runtime state directories
  (`store_root` must be a local filesystem: session overlays need overlayfs, which cannot write to NFS. Session files themselves also work on NFS; see ADR 0012.)
