...
Set `store_root` and `cold_root` in the config file, not through environment variables or flags. The sudo mount helpers read only the root-owned config file, and they must see the same roots as the service.
...
- Mount it before Aperture starts. Aperture creates `cold_root/snapshots` and `cold_root/sessions` when missing, so an unmounted share would be silently replaced by empty local directories.
...
The command refuses to run while any session overlay is mounted. On the same filesystem it renames each entry. Across filesystems it copies each snapshot and session files directory to a staging name next to its destination, verifies every file byte for byte, renames the copy into place, and removes the sources only after every entry is published. It keeps permissions, modification times, symlinks, and the hard links snapshots share, and it updates the paths recorded in the database. An interrupted run can be repeated; entries that already exist at the destination are verified against their source instead of copied again, and a mismatch stops the migration.
