...
- A marker with another ID: refuse to start. `cold_root` holds another install's data, or the database was replaced. Remove the marker only when that is intended.
...
- Mount it before Aperture starts. Once the install has data, the missing marker of an unmounted share stops Aperture from starting; a fresh install started before the mount marks the bare mount point instead.
