...
Every nightly prerelease also publishes the packages, at the nightly version (`0.0.0-nightly.<run>.<commit>`) under the `nightly` dist-tag, so `npm install @aperture-browser/api-client@nightly` gets the latest `master` build. The nightly workflow creates its release with the GitHub App token, because a release created with `GITHUB_TOKEN` does not start `npm.yml`.
...
These runs share the default branch's cache scope, so they only restore caches, which the nightly workflow saves. Nix builds pull from the public [`aperture-browser` Cachix cache](https://app.cachix.org/cache/aperture-browser). Only the Docker image workflow on `master` pushes to it, and only the custom builds in the `docker-build-deps` flake output, using the `CACHIX_AUTH_TOKEN` secret.
