...
func TestCDPForwardAuthRejectsAuthorizationHeaderToken(t *testing.T) {
	...
	rec := env.doRaw(t, http.MethodGet, "/internal/forward-auth/cdp/"+created.Session.ID, map[string]string{
		"Authorization": "Bearer " + created.Session.Connection.SessionToken,
	}, nil)
	if rec.Code != http.StatusUnauthorized {
		t.Fatalf("status = %d, want %d", rec.Code, http.StatusUnauthorized)
	}
...
func TestCDPForwardAuthAllowsPathToken(t *testing.T) {
	...
	rec := env.doRaw(t, http.MethodGet, "/internal/forward-auth/cdp/"+created.Session.ID, map[string]string{
		"X-Forwarded-Uri": "/sessions/" + created.Session.ID + "/cdp/" + created.Session.Connection.SessionToken + "/devtools/browser/test",
	}, nil)
...
func TestCDPForwardAuthRejectsQueryToken(t *testing.T) {
	...
	rec := env.doRaw(t, http.MethodGet, "/internal/forward-auth/cdp/"+created.Session.ID, map[string]string{
		"X-Forwarded-Uri": "/sessions/" + created.Session.ID + "/cdp/devtools/browser/test?token=" + created.Session.Connection.SessionToken,
	}, nil)
	if rec.Code != http.StatusUnauthorized {
		t.Fatalf("status = %d, want %d", rec.Code, http.StatusUnauthorized)
	}
