package httpapi

import (
	"context"
	"encoding/json"
	"net/http"
	"os"
	"path/filepath"
	"testing"

	"github.com/aperture/aperture/internal/auth"
	"github.com/aperture/aperture/internal/browser"
	"github.com/aperture/aperture/internal/config"
	"github.com/aperture/aperture/internal/db"
	"github.com/aperture/aperture/internal/paths"
	"github.com/aperture/aperture/internal/session"
	"github.com/aperture/aperture/internal/supervisor"
	"github.com/aperture/aperture/internal/systemd"
	"github.com/aperture/aperture/internal/traefik"
	"go.uber.org/zap"
)

func newSessionTestEnv(t *testing.T) *testEnv {
	t.Helper()

	env := newTestEnv(t)
	root := t.TempDir()
	cfg := config.Config{
		StoreRoot:                filepath.Join(root, "store"),
		RuntimeRoot:              filepath.Join(root, "runtime"),
		ArtifactRoot:             filepath.Join(root, "artifacts"),
		DatabasePath:             filepath.Join(root, "unused.db"),
		TraefikDynamicConfigPath: filepath.Join(root, "runtime", "traefik", "dynamic.yaml"),
		ListenAddress:            "127.0.0.1:8080",
		SystemdBrowserUnitName:   "browser-session@.service",
		SessionRetentionDays:     7,
		SnapshotRetentionDays:    7,
		ChannelRegistry: map[string]config.ChannelConfig{
			"chromium": {Executable: "/usr/bin/chromium"},
		},
		ExternalBaseURL:  "https://browser.example.test",
		CdpRouteBasePath: "/cdp",
		LogLevel:         "info",
	}

	channels, err := browser.NewRegistry(cfg)
	if err != nil {
		t.Fatalf("registry: %v", err)
	}
	runner := &sessionHandlerFakeRunner{active: make(map[string]bool)}
	browserSupervisor, err := supervisor.NewBrowser(cfg, runner)
	if err != nil {
		t.Fatalf("browser supervisor: %v", err)
	}
	sessions := session.NewService(cfg, env.repo, &sessionHandlerFakeOverlay{cfg: cfg}, browserSupervisor, channels, traefik.NoopReconciler{})

	server := &Server{Auth: env.service, Sessions: sessions, Channels: channels}
	env.router = NewRouter(zap.NewNop(), server, nil, cfg.CdpRouteBasePath)
	return env
}

type sessionHandlerFakeOverlay struct {
	cfg config.Config
}

func (f sessionHandlerFakeOverlay) Mount(_ context.Context, sessionID string, _ *string) error {
	layout, err := paths.Session(f.cfg, sessionID)
	if err != nil {
		return err
	}
	for _, dir := range []string{layout.Upper, layout.Work, layout.Merged, layout.Downloads, layout.Cache} {
		if err := os.MkdirAll(dir, 0o755); err != nil {
			return err
		}
	}
	return nil
}

func (sessionHandlerFakeOverlay) Unmount(context.Context, string) error { return nil }

type sessionHandlerFakeRunner struct {
	active map[string]bool
}

func (f *sessionHandlerFakeRunner) Run(ctx context.Context, name string, args ...string) ([]byte, error) {
	if len(args) >= 3 && args[0] == "--user" && args[1] == "start" {
		f.active[args[2]] = true
	}
	if len(args) >= 3 && args[0] == "--user" && args[1] == "stop" {
		delete(f.active, args[2])
	}
	if len(args) >= 3 && args[0] == "--user" && args[1] == "is-active" {
		if f.active[args[2]] {
			return []byte("active\n"), nil
		}
		return nil, &systemd.CommandError{ExitCode: 3}
	}
	return []byte("inactive\n"), nil
}

func TestSessionLifecycleHandlers(t *testing.T) {
	t.Parallel()

	env := newSessionTestEnv(t)

	sessionsToken, err := env.service.CreateToken(context.Background(), auth.CreateTokenInput{
		AuthorityType: auth.AuthorityTenant,
		TenantID:      &env.tenantID,
		Name:          "sessions-operator",
		Scopes:        []string{auth.ScopeSessionsWrite},
	})
	if err != nil {
		t.Fatalf("create sessions token: %v", err)
	}

	createRec := env.do(t, http.MethodPost, "/api/sessions", sessionsToken.Raw, "", map[string]any{
		"browser": map[string]any{
			"channel": "chromium",
			"args":    []string{},
		},
	})
	if createRec.Code != http.StatusCreated {
		t.Fatalf("create session status = %d, body = %s", createRec.Code, createRec.Body.String())
	}

	var created createSessionResponse
	if err := json.Unmarshal(createRec.Body.Bytes(), &created); err != nil {
		t.Fatalf("decode create response: %v", err)
	}
	if created.Session.Status != db.SessionStatusRunning {
		t.Fatalf("status = %q, want running", created.Session.Status)
	}

	deleteRec := env.do(t, http.MethodDelete, "/api/sessions/"+created.Session.ID, sessionsToken.Raw, "", nil)
	if deleteRec.Code != http.StatusOK {
		t.Fatalf("delete session status = %d, body = %s", deleteRec.Code, deleteRec.Body.String())
	}

	reopenRec := env.do(t, http.MethodPost, "/api/sessions/"+created.Session.ID+"/reopen", sessionsToken.Raw, "", nil)
	if reopenRec.Code != http.StatusOK {
		t.Fatalf("reopen session status = %d, body = %s", reopenRec.Code, reopenRec.Body.String())
	}
}

func TestCreateSessionFromSnapshotRequiresSnapshotsReadScope(t *testing.T) {
	t.Parallel()

	env := newSessionTestEnv(t)
	ctx := context.Background()

	snapshotID := "018f1234-0000-7000-8000-000000000050"
	if err := env.repo.CreateSnapshot(ctx, &db.Snapshot{
		ID:        snapshotID,
		TenantID:  env.tenantID,
		Name:      "github-main",
		Path:      "/store/snapshots/01/8f/" + snapshotID + "/profile",
		CreatedAt: db.NowUTC(),
	}); err != nil {
		t.Fatalf("create snapshot: %v", err)
	}

	writeOnly, err := env.service.CreateToken(ctx, auth.CreateTokenInput{
		AuthorityType: auth.AuthorityTenant,
		TenantID:      &env.tenantID,
		Name:          "sessions-write-only",
		Scopes:        []string{auth.ScopeSessionsWrite},
	})
	if err != nil {
		t.Fatalf("create write-only token: %v", err)
	}

	denied := env.do(t, http.MethodPost, "/api/sessions", writeOnly.Raw, "", map[string]any{
		"baseSnapshotName": "github-main",
		"browser": map[string]any{
			"channel": "chromium",
			"args":    []string{},
		},
	})
	if denied.Code != http.StatusForbidden {
		t.Fatalf("status = %d, want %d, body = %s", denied.Code, http.StatusForbidden, denied.Body.String())
	}

	readWrite, err := env.service.CreateToken(ctx, auth.CreateTokenInput{
		AuthorityType: auth.AuthorityTenant,
		TenantID:      &env.tenantID,
		Name:          "sessions-and-snapshots",
		Scopes:        []string{auth.ScopeSessionsWrite, auth.ScopeSnapshotsRead},
	})
	if err != nil {
		t.Fatalf("create read/write token: %v", err)
	}

	allowed := env.do(t, http.MethodPost, "/api/sessions", readWrite.Raw, "", map[string]any{
		"baseSnapshotName": "github-main",
		"browser": map[string]any{
			"channel": "chromium",
			"args":    []string{},
		},
	})
	if allowed.Code != http.StatusCreated {
		t.Fatalf("status = %d, want %d, body = %s", allowed.Code, http.StatusCreated, allowed.Body.String())
	}
}

func TestCreateSessionRejectsDeniedBrowserArgs(t *testing.T) {
	t.Parallel()

	env := newSessionTestEnv(t)
	token, err := env.service.CreateToken(context.Background(), auth.CreateTokenInput{
		AuthorityType: auth.AuthorityTenant,
		TenantID:      &env.tenantID,
		Name:          "sessions-deny",
		Scopes:        []string{auth.ScopeSessionsWrite},
	})
	if err != nil {
		t.Fatalf("create token: %v", err)
	}

	rec := env.do(t, http.MethodPost, "/api/sessions", token.Raw, "", map[string]any{
		"browser": map[string]any{
			"channel": "chromium",
			"args":    []string{"--remote-debugging-port=9222"},
		},
	})
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("status = %d, want %d, body = %s", rec.Code, http.StatusBadRequest, rec.Body.String())
	}
}
