...
type wrapperCDPError struct {
	Code    int    `json:"code"`
	Message string `json:"message"`
}

type wrapperCDPEnvelope struct {
	ID        int64            `json:"id"`
	SessionID string           `json:"sessionId"`
	Method    string           `json:"method"`
	Params    json.RawMessage  `json:"params"`
	Result    json.RawMessage  `json:"result"`
	Error     *wrapperCDPError `json:"error"`
}

type wrapperCDPClient struct {
	ctx        context.Context
	...
	mu      sync.Mutex
	nextID  int64
	pending map[int64]chan wrapperCDPEnvelope
	writeMu sync.Mutex
	events  chan wrapperCDPEnvelope
	done    chan error
}

func newWrapperCDPClient(ctx context.Context, port int) (*wrapperCDPClient, error) {
	webSocketURL, err := browserCDPWebSocketURL(ctx, port)
	if err != nil {
		return nil, err
	}
	clientCtx, cancel := context.WithCancel(ctx)
	connection, _, err := websocket.Dial(clientCtx, webSocketURL, nil)
	if err != nil {
		cancel()
		return nil, fmt.Errorf("connect browser CDP endpoint: %w", err)
	}
	connection.SetReadLimit(cdpDiscoveryMessageLimit)
	client := &wrapperCDPClient{
		ctx:        clientCtx,
		cancel:     cancel,
		connection: connection,
		pending:    make(map[int64]chan wrapperCDPEnvelope),
		events:     make(chan wrapperCDPEnvelope, 32),
...
func (client *wrapperCDPClient) readLoop() {
	var readErr error
	defer func() {
		client.mu.Lock()
		pending := client.pending
		client.pending = make(map[int64]chan wrapperCDPEnvelope)
		client.mu.Unlock()
	...
	for {
		...
		var envelope wrapperCDPEnvelope
...
func (client *wrapperCDPClient) Call(ctx context.Context, sessionID string, method string, params any, result any) error {
...
func (client *wrapperCDPClient) Send(ctx context.Context, sessionID string, method string, params any) error {
	_, _, err := client.send(ctx, sessionID, method, params, false)
	return err
}

func (client *wrapperCDPClient) send(ctx context.Context, sessionID string, method string, params any, wait bool) (int64, <-chan wrapperCDPEnvelope, error) {
	client.mu.Lock()
	client.nextID++
	requestID := client.nextID
	var response chan wrapperCDPEnvelope
	if wait {
		response = make(chan wrapperCDPEnvelope, 1)
...
func (client *wrapperCDPClient) removePending(requestID int64) {
	client.mu.Lock()
	delete(client.pending, requestID)
	client.mu.Unlock()
}

func (client *wrapperCDPClient) Close() {
	client.cancel()
...
func setTargetDeviceMetrics(ctx context.Context, port int, targetID string, viewport compositorViewport) error {
	client, err := newWrapperCDPClient(ctx, port)
	if err != nil {
		return err
	}
	defer client.Close()
