...
/**
 * Runs inside the top frame through chrome.scripting.executeScript, after capture-codec.js.
 * Like capturePageState, it must not reference anything outside its body.
 */
export async function captureDocumentState(codecKey: string): Promise<CapturedDocumentState> {
  ...
  function elementLocator(element: HTMLElement): InitialBrowserElementLocator {
    ...
    const path: Array<InitialBrowserElementLocator["path"][number]> = [];
  ...
  const controls: Array<InitialBrowserDocumentState["controls"][number]> = [];
  ...
  const contentEditables: Array<InitialBrowserDocumentState["contentEditables"][number]> = [];
  ...
  const scrollPositions: Array<InitialBrowserDocumentState["scrollPositions"][number]> = [];
